...
def extract_all_captions(captions, model, device, batch_size=1024, num_workers=8):
...
def extract_all_images(images, model, device, batch_size=1024, num_workers=8):
    data = torch.utils.data.DataLoader(
...
def main():
    ...
    candidates_json = "/nethome/song0018/clip_prefix_caption/data/test_sis_sents_imgid_gpt2xl_nocon_num1-004.json"
    ...
    image_ids = list(candidates_all.keys())


    #for decoding_mode in ['beam', 'topk', 'nucleus', 'contrastive']:
    for decoding_mode in ['true']:
        ...
        print(f"============================== {decoding_mode} ====================================")
        # candidates = [candidates_all[cid][decoding_mode][0] for cid in image_ids]
        # references = [candidates_all[cid]['true'] for cid in image_ids]
        # print(candidates[100:102])
        # print(references[100:102])
        # sys.exit(0)
        candidates = []
        references = []
        new_image_ids = []
        for cid in image_ids:
            if candidates_all[cid][decoding_mode] != []:
                num = len(candidates_all[cid][decoding_mode])
                for i in range(num):
                    candidates.append(candidates_all[cid][decoding_mode][i])
                    references.append(candidates_all[cid]['true'])
                    new_image_ids.append(cid)
            else:
                candidates.append(candidates_all[cid]['true'][0])
                references.append(candidates_all[cid]['true'])
                new_image_ids.append(cid)
        ...
        #     with open(save_per_instance, 'w') as f:
...
# #######################  Load data  #############################
# data_dir = '/scratch/song0018/vist_data/results/'
# #data_dir = '/hpc/shared/uu_vl/yingjin_datasets/vist_data/clipcap_models/results/'
#
# img_dir = '/scratch/song0018/vist_data/images/test_images'
#
#
# with open(os.path.join(data_dir, 'test_sis_sents_imgid_1024_contras_concat_gpt2l-009.json'), 'r') as json_file:
#     imgid_sents_data = json.load(json_file)
